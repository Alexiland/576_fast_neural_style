...
class QConv2d(nn.Conv2d):
    ...
    def __init__(self, in_channels, out_channels, kernel_size,
                 stride=1, padding=0, dilation=1, groups=1, bias=True, momentum=0.1, quant_act_forward=0,
                 quant_act_backward=0, quant_grad_act_error=0, quant_grad_act_gc=0, weight_bits=0, fix_prec=False):
        ...
        self.stride = stride
        self.num_bits = 0
        self.num_grad_bits = 0

    def forward(self, input):
        num_bits = self.num_bits
        num_grad_bits = self.num_grad_bits
        if num_bits == 0:
        ...
        if self.fix_prec:
            if self.quant_act_forward or self.quant_act_backward or self.quant_grad_act_error or self.quant_grad_act_gc or self.weight_bits:
                weight_qparams = calculate_qparams(self.weight, num_bits=self.weight_bits, flatten_dims=(1, -1),
                                                   reduce_dim=None)
                ...
                output = self.conv2d_quant_act(qinput_fw, qinput_bw, qweight, qbias, self.stride, self.padding,
                                               self.dilation, self.groups, error_bits, gc_bits)
