...
def count_convNd_bnn(m: _ConvNd, x: (torch.Tensor,), y: torch.Tensor):
    x = x[0]

    # kernel_ops = torch.zeros(m.weights.size()[2:]).numel()  # Kw x Kh
    kernel_ops = m.num_kernel  # Kw x Kh
...
register_hooks = {
    ...
    HardBinaryConv: count_convNd_bnn,
...
def profile(model: nn.Module, inputs, custom_ops=None, verbose=True):
    ...
    def dfs_count(module: nn.Module, prefix="\t", is_float = False) -> (int, int):
